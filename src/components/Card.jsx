...
export default function Card({ card, onOpen, preview = false, onContextMenu }) {
  const platform = PLATFORMS[card.platform] || PLATFORMS.youtube

  return (
    <div
      onClick={preview ? undefined : () => onOpen(card)}
      onContextMenu={
      ...
      className={`group relative flex flex-col overflow-hidden rounded-2xl border border-white/[0.07] bg-[#121216] transition-all duration-300 ${
        preview ? '' : 'cursor-pointer hover:-translate-y-1.5'
    ...
    >
      ...
      <div className="relative h-40 overflow-hidden">
        ...
        <div
          className={`absolute inset-0 flex items-center justify-center bg-black/40 ${
            preview ? '' : 'opacity-0 transition-opacity duration-300 group-hover:opacity-100'
          }`}
        >
          <div
            className="flex h-12 w-12 items-center justify-center rounded-full backdrop-blur transition-transform duration-300 group-hover:scale-110"
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        <div className="mt-1 flex items-center justify-between border-t border-white/[0.06] pt-3">
          ...
          {!preview && (
            <span
              className="flex items-center gap-1 text-[12px] font-semibold text-gray-300 transition-colors group-hover:text-white"
            >
              Watch now
              <ArrowUpRight
                size={13}
